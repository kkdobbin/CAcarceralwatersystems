#Iteratively scrape CCRs for all systems. Code by Liza Wood
...
AdditionalCCRs <- read_csv("Data/additionalCCRpullsneeded.csv")
CASDWIS <- read_csv("Data/CASDWIS_WaterSystemSummary_20250812.csv")
CASDWIS <- CASDWIS[,c(1,2)]
AdditionalCCRs <- left_join(AdditionalCCRs, CASDWIS, by = c("PWSID" = "Water System No."))
AdditionalCCRs <- AdditionalCCRs %>% filter(!is.na(`Water System Name`)) #get rid of non active systems
ids <- AdditionalCCRs$PWSID

# Honestly everything else from here should run on its own in your computer
# What you'll end with is your storage path folder all full of reports
...
N_CCR_both <- full_join(N_CCR, N_CCR2)
N_CCR_both <- N_CCR_both %>% replace(is.na(.), 0)

write_csv(N_CCR, "Data/NCCR_secondbatch.csv")
write_csv(N_CCR2, "Data/NCCR2_secondbatch.csv")
write_csv(log, "Data/CCR_log_secondbatch.csv")
write_csv(N_CCR_both, "Data/NCCR_full_secondbatch.csv")

NCCR_full_firstbatch <- read_csv("Data/NCCR_full.csv")
NCCR_master <- rbind(NCCR_full_firstbatch, N_CCR_both)
write_csv(NCCR_master, "Data/NCCR_full_master.csv")
